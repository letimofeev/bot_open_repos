...
class IFileLoader(ABC):
    """
    Интерфейс для класса загрузчика файлов
    """
    __metaclass__ = ABCMeta

    @abstractmethod
    def get_csv(self, file_name: str, folder_name: str) -> pd.DataFrame:
        raise NotImplementedError

    @abstractmethod
    def update_csv(self, file_name: str, df: pd.DataFrame) -> bool:
        raise NotImplementedError

    @abstractmethod
    def get_excel(self, file_name: str, sheet_name: str, folder_name: str) -> pd.ExcelFile or pd.DataFrame:
        raise NotImplementedError


class PandasLoader(IFileLoader):
    ...
    """
    def __init__(self, storage=GoogleAPI):
    ...
    def get_excel(self, file_name: str, sheet_name: str = None, folder_name: str = None) -> pd.ExcelFile or pd.DataFrame:
        ...
        if self.is_exist(file_name):
            if sheet_name is not None:
                pd.ExcelFile(file_name).parse(sheet_name)
            else:
